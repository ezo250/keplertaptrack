      ...
      <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
        ...
        {mySchedule.length > 0 && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.35 }}
            className="relative overflow-hidden bg-gradient-to-br from-card via-card/95 to-primary/5 rounded-xl border border-border/50 p-4 shadow-lg"
          >
            {/* Decorative background elements */}
            <div className="absolute top-0 right-0 w-48 h-48 bg-gradient-to-br from-primary/5 to-transparent rounded-full blur-3xl -z-0" />
            <div className="absolute bottom-0 left-0 w-32 h-32 bg-gradient-to-tr from-secondary/5 to-transparent rounded-full blur-3xl -z-0" />
            
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-4 pb-3 border-b border-border/30">
                <motion.div 
                  whileHover={{ rotate: 5, scale: 1.05 }}
                  className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary via-primary to-primary/80 flex items-center justify-center shadow-md"
                >
                  <Calendar className="w-5 h-5 text-white" />
                </motion.div>
                <div className="flex-1">
                  <h2 className="text-base sm:text-lg font-heading font-bold text-foreground bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text">
                ...
                {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map((day, dayIndex) => {
                  ...
                  return (
                    <motion.div 
                      ...
                      className={`rounded-xl border overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 ${
                        isToday 
                    ...
                    >
                      <div className={`px-4 py-2.5 border-b ${
                        ...
                        <div className="flex items-center justify-between">
                          <h3 className="font-bold text-foreground flex items-center gap-2">
                            <span className={`text-sm sm:text-base ${isToday ? 'text-primary' : 'text-foreground/80'}`}>
                              {day}
                            </span>
                            {isToday && (
                              <motion.span 
                                ...
                                className="text-xs px-2 py-0.5 rounded-full bg-primary text-white font-semibold"
                              >
                                Today
                              </motion.span>
                            )}
                      ...
                      <div className="p-3 bg-gradient-to-br from-card via-card/98 to-muted/5 space-y-3">
                        {dayClasses.sort((a, b) => a.startTime.localeCompare(b.startTime)).map((classItem, index) => {
                          ...
                          return (
                            <motion.div
                              ...
                              className={`relative p-3 rounded-lg border transition-all duration-300 space-y-2.5 group ${
                                isHappeningNow
                                  ? 'bg-gradient-to-br from-primary/10 via-primary/5 to-card border-primary/40 shadow-md ring-1 ring-primary/20'
                                  : isUpcoming
                            ...
                            >
                                ...
                                <div className="absolute top-2 right-2">
                                  {isHappeningNow ? (
                                    <motion.span 
                                      ...
                                      className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-primary text-white text-xs font-bold"
                                    >
                                      <span className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />
                                      In Progress
                                    </motion.span>
                                  ) : isUpcoming ? (
                                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-secondary/20 text-secondary text-xs font-semibold border border-secondary/30">
                              ...
                              {/* Time */}
                              <div className="flex items-center gap-2 text-foreground pl-2">
                                <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${
                                  isHappeningNow
                                    ? 'bg-primary/20 group-hover:bg-primary/30 ring-1 ring-primary/30'
                                    : 'bg-primary/10 group-hover:bg-primary/20'
                                }`}>
                                  <Clock className={`w-3.5 h-3.5 ${isHappeningNow ? 'text-primary' : 'text-primary/80'}`} />
                                </div>
                                <div>
                              ...
                              <div className="flex items-center gap-2 pl-2">
                                <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all ${
                                  isHappeningNow
                                    ? 'bg-secondary/20 group-hover:bg-secondary/30 ring-1 ring-secondary/30'
                                    : 'bg-secondary/10 group-hover:bg-secondary/20'
                                }`}>
                                  <GraduationCap className="w-3.5 h-3.5 text-secondary" />
                                </div>
                                <span className="text-sm font-heading font-bold text-foreground leading-tight">{classItem.course}</span>
                              </div>

                              {/* Classroom */}
                              {classItem.classroom && (
                                <div className="flex items-center gap-2 pl-2">
                                  <div className={`flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 w-fit transition-all ${
                                    isHappeningNow
                                      ? 'bg-gradient-to-r from-primary/20 to-primary/15 ring-1 ring-primary/20'
